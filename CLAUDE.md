...
# Tests — 307 tests in mainapp/tests.py
...
* Destinations span **every school year**, because promoting a student is the case that made the wrong-year enrolment bug matter. `_destination_groups()` orders in the database rather than through `sort_key_section`, which raises on any `Section` not shaped `<digit><letter>`.
...
Arriving at `assign_subjects` or `reassign_students` with only `?course_id=` recovers the type and level **server-side** from the course row. That is what the never-implemented `LEVEL_LOOKUP` mode was for; the branch is dropped and its job is now the ordinary path. `reassign_students` goes further and lets the row **override** a disagreeing `?school_year_id=` rather than only filling a gap, on the same reasoning `resolve_class_scope` uses: a year that disagrees with the course describes a class that does not exist.
