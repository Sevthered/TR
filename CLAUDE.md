...
> [!key-insight] Never render a disagreement — resolve it or refuse it
> Three separate bugs this codebase has shipped were one defect: **a page rendering two facts about one scope, derived from separate sources, with nothing forcing them to agree.** `assign_subjects` said "Sección seleccionada · Ninguna" above that section's roster; `reassign_students` named a course in its heading with Nivel empty beneath it; `assign_subjects` again showed a year badge above a section badge from a different year.
>
> What makes it a class rather than three mistakes: **every individual value is correct.** Nothing miscomputes. The defect lives only in the *relation* between two values, which no expression evaluates — so a value-level test cannot catch it and a green suite is guaranteed. All three were found by rendering the page and reading it; none by a test. It scales with the number of scope inputs, not with complexity.
>
> **The rule has two halves.** *(a) One resolver* — a page's scope is resolved once into one object and everything renders from that object. `class_dashboard` → `resolve_class_scope()` → `ClassScope` is the worked example, and it is the page this never happened on. The administrator flows have no resolver, only loose locals threaded into a context dict, and `target_course` never reaching the template is that shape's characteristic failure. *(b) A conflict is resolved or refused, never carried forward.*
>
> **Which of the two depends on whether an honest request can produce the disagreement**, and the codebase now shows both answers on purpose. `reassign_students` **drops the course and keeps the year**, because with JavaScript off the section select resubmits itself untouched, so a stale `course_id` arrives from a perfectly honest submit and the year is the control a person just operated. `import_grades` and `assign_subjects` **refuse**, because a CSV row is data whose impossible year is a mistake, and because `assign_subjects` renders `school_year_id` as a hidden input with the cascade's `hx-include` scoped to it — so no honest request can produce the mismatch there and the only caller that can built the POST by hand. Do **not** flatten these into one behaviour; the divergence is the rule being applied, not an inconsistency.
>
> **The testing corollary is the part that would have caught all three: assert the relation, not the values.** `test_no_assignment_ever_spans_two_school_years` walks every row asserting `course.school_year_id == trimester.school_year_id` and never mentions a repro, so it survives rewording and catches the whole class. A test that the year in the header badge equals the year of the course in the section badge is the rendering equivalent.
...
**Every template in the app now extends `base_v2` or `base_shell_v2`** — `LegacyCascadeTeardownTests.test_every_template_is_on_the_v2_cascade` states that invariant directly, exempting only `base_v2.html` itself and the five `_`-prefixed partials. It asserts on `{% extends %}` and deliberately *not* on the string `<!DOCTYPE`: several migrated pages name the doctype they dropped in a `{% comment %}`, which is the same way the old `global-styles.css` guard came to pass against a comment.
...
- ~~The whole nav is desktop-only~~ — **fixed 2026-08-03.** Below 768px a phone rendered the brand bar and *nothing else of the shell*: zero nav links visible at 500px, for every role. It is a `<details>` drawer now, and the shape of the fix matters more than the fix. **There are two navs and they are exact complements** — the drawer carries `md:hidden`, the 224px column `hidden md:*` — so both are in the DOM at once and exactly one renders. Consequences: (a) the links live in `templates/_nav_links.html`, included twice, and **nothing in that partial may carry an `id`**, which is why the nav's one id (`class-enrolled`, an out-of-band swap target) stays in the shell's desktop-only "Esta clase" block; (b) the assertion that keeps the two copies honest is a *relation* — `NavDrawerTests.test_both_widths_offer_the_same_destinations` compares the two href sets rather than checking either one, per the *never render a disagreement* rule above. A disclosure was chosen over a scrolling row or a tab bar because it needs no JavaScript, and under `script-src 'self'` a blocked script fails silently.
