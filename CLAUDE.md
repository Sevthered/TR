...
- **The whole nav is desktop-only.** Every role's link block in `base_shell_v2.html` is `hidden md:block` and the identity footer is `hidden md:flex`, so below 768px a phone renders the brand bar and *nothing else of the shell*. Verified in a browser: zero nav links visible at 500px, for every role. A professor cannot reach "Mis clases" or any section; an administrator cannot reach any of the six admin flows. A logout control was added to the brand bar on 2026-08-03 and is currently the **only** nav affordance mobile has. This needs a design decision (drawer? collapsed row? top-bar menu?) more than it needs code, and it is its own slice.
